...
## System 3+4 Governance Addendum

### Lazy Load (required)

1. Read this file first, then `CLAUDE.md`.
2. Route by task:
   - UI work -> `Quotio/Views/AGENTS.md` + `Quotio/Views/CLAUDE.md`
   - Service/runtime work -> `Quotio/Services/AGENTS.md` + `Quotio/Services/CLAUDE.md`
   - build/release work -> `scripts/AGENTS.md` + `scripts/CLAUDE.md`
3. Do not pre-read unrelated modules.

### Search Before Writing (required)

```bash
rg -n "keyword|symbolName" Quotio scripts
rg --files Quotio scripts | rg "keyword"
```

If a reusable implementation exists, reuse it. If not reused, state why in the delivery report.

Use this reporting snippet:
```text
[Reuse Decision]
- Search keywords:
- Reused path (if any):
...
- Never hardcode or print secrets in code/logs/commit messages.
- Before commit, run:
...
### IaC Consistency Status

- Current gap: no unified DevContainer/Nix entry for `quotio/`.
- Minimum landing path: keep Xcode-native flow, document optional containerized helpers in module docs and align with root governance docs.
