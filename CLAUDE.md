...
## 8) System 3+4 Addendum

### 8.1 Lazy Load

1. Read `AGENTS.md` then this file.
2. Load only the target sub-module guide:
   - `Quotio/Views/*` for UI tasks
   - `Quotio/Services/*` for runtime/service tasks
   - `scripts/*` for release/build tasks

### 8.2 Search Before Writing
...
### 8.3 Secret Source and Gate

- Real secrets are allowed only from root `.env` or process ENV.
- Never expose keys in source/logs/commit messages.
- Run before commit:
...
- `quotio/` currently has no unified DevContainer/Nix entry.
- Keep Xcode local workflow as primary path and sync environment constraints in docs when changed.
